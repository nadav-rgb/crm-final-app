// lib/aiDemo.js
// שכבת סיכום מובנה מבוססת ניתוח מילות-מפתח (ללא קריאת שרת/עלויות).
// מפיקה סיכום קריא לרכז לפי שדות העבודה: מצב הקשר, התקדמות רוחנית, פעולה מומלצת ודגלים.
// (סיכום AI אמיתי לדיווחי קשר זמין דרך lib/aiService.js + api/ai-summary.)
...
function pickRelationshipStatus(text) {
  ...
  return 'ראשוני: הקשר תועד, אך עדיין לא זוהתה רמת עומק מספקת.';
}
...
function pickCoordinatorFlags(text) {
  ...
  if (flags.length === 0) flags.push('אין דגל חריג, אך מומלץ לוודא שיש פעולה הבאה ברורה.');
  return flags;
}
...
function buildStructuredSummary({ title, text, contextLine }) {
  ...
  return [
    ...
    '• הערה: הסיכום מבוסס על ניתוח מילות המפתח שבטקסט הדיווח.'
  ].filter(Boolean).join('\n');
}
...
export function generateMeetingNotesAiSummaryDemo(house) {
  ...
  return [
    `סיכום לארבעת המפגשים — בית מפגש ${houseLabel}:`,
    '',
    ...
    '• הערה: הסיכום מבוסס על ניתוח מילות המפתח שבהערות המפגשים.',
  ].join('\n');
}
...
export function summarizeMeetingHouseSeriesDemo(house, reports = []) {
  ...
  return [
    ...
    `• דגלים לרכז: ${flags.join(' ')}`,
    '• הסיכום מבוסס על הדיווחים שהוזנו למפגשים.'
  ].join('\n');
}
