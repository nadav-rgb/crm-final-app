...
const TYPE_ROWS = [
  { key: 'phone-friendly', label: 'טלפוני ידידותי',      type: 'טלפוני',    quality: 'ידידותי',    priceKey: 'טלפוני-ידידותי' },
  { key: 'phone-torani', label: 'טלפוני תורני',          type: 'טלפוני',    quality: 'תורני',      priceKey: 'טלפוני-תורני' },
  { key: 'video-friendly', label: 'זום ידידותי',          type: 'וידאו',     quality: 'ידידותי',    priceKey: 'וידאו-ידידותי' },
  { key: 'video-torani', label: 'זום תורני',              type: 'וידאו',     quality: 'תורני',      priceKey: 'וידאו-תורני' },
  { key: 'frontal-friendly', label: 'פרונטלי ידידותי',   type: 'פרונטלי',   quality: 'ידידותי',    priceKey: 'פרונטלי-ידידותי' },
  { key: 'frontal-torani', label: 'פרונטלי תורני',       type: 'פרונטלי',   quality: 'תורני',      priceKey: 'פרונטלי-תורני' },
  { key: 'frontal-multi', label: 'פרונטלי רב משתתפים',   type: 'פרונטלי',   quality: 'רב משתתפים', priceKey: 'פרונטלי-רב משתתפים' },
  { key: 'shabbat-hosting', label: 'אירוח שבת',           type: 'אירוח שבת', quality: null,         priceKey: 'אירוח שבת' },
];
...
// Adapter for the hardened Finance DTO. It deliberately consumes aggregate rows only:
// no contact identifier, name, note, interaction, or religious-history field reaches
// the browser workbook path.
function deriveActivityByTypeFromPayment(payment = {}) {
  const finite = (value) => Number.isFinite(Number(value)) ? Number(value) : 0;
  const activityByKey = new Map((payment.activityByType || []).map((row) => [row.key, row]));
  const typeRows = TYPE_ROWS.map((type) => {
    const aggregate = activityByKey.get(type.key) || {};
    return {
      label: type.label,
      count: Math.max(0, Number.parseInt(aggregate.count ?? 0, 10) || 0),
      rate: finite(aggregate.unitRate),
      total: finite(aggregate.total),
    };
  });
  const meetingsTotal = typeRows.reduce((sum, row) => sum + row.total, 0);

  const aggregates = Array.isArray(payment.bonusByType) ? payment.bonusByType : [];
  const bonusRows = BONUS_CATEGORIES.map((category) => {
    const matching = aggregates.filter((row) => category.types.includes(row.type));
    const count = matching.reduce((sum, row) => sum + Math.max(0, Number.parseInt(row.count ?? 0, 10) || 0), 0);
    if (count === 0) return null;
    return {
      label: category.label,
      count,
      detail: '',
      amount: matching.reduce((sum, row) => sum + finite(row.total), 0),
    };
  }).filter(Boolean);

  const expensesTotal = finite(payment.expenseTotal);
  const guidePay = finite(payment.tourTotal);
  const expensesRow = expensesTotal > 0 ? { label: 'החזר הוצאות', amount: expensesTotal } : null;
  const guideRow = guidePay > 0 ? { label: 'הדרכת סיורים', amount: guidePay } : null;
  const unpaidByReason = (payment.unpaidByReason || []).map((row) => ({
    reason: String(row.label || row.reason || ''),
    count: Math.max(0, Number.parseInt(row.count ?? 0, 10) || 0),
  }));
  const unpaidCount = unpaidByReason.reduce((sum, row) => sum + row.count, 0);
  const bonusTotal = bonusRows.reduce((sum, row) => sum + row.amount, 0)
    + (expensesRow?.amount ?? 0) + (guideRow?.amount ?? 0);
  ...
  return {
    ...
    expensesRow,
    guideRow,
    grandTotal: meetingsTotal + bonusTotal,
    unpaidCount,
    unpaidByReason,
    detailByType: Object.fromEntries(TYPE_ROWS.map((type) => [type.label, []])),
  };
}
...
module.exports = {
  deriveActivityByType, deriveActivityByTypeFromPayment, TYPE_ROWS, BONUS_CATEGORIES,
