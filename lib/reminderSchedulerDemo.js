...
// רצף התזכורות לדיווחי מפגשי בסיס: 23:00 → 10:00 → 11:30 → 12:00 (הסלמה לרכז).
// כל שלב יוצר התראת אמת (createDemoNotification → Supabase). התזמון בפועל מנוהל ב-api/reminders.
...
function createActivistReminder(report, nextStage) {
  const stageTexts = {
    [REMINDER_STAGES.NIGHT_23]: {
      ...
      body: `נא למלא דיווח עבור ${buildReportTitle(report)}. תזכורת ערב (23:00).`,
    },
    [REMINDER_STAGES.MORNING_10]: {
      title: 'תזכורת נוספת למילוי דיווח',
      body: `עדיין לא מולא דיווח עבור ${buildReportTitle(report)}. תזכורת בוקר (10:00).`,
    },
