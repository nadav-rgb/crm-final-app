...
// דיווח מפגש בסיס התקבל — התראה אחת לכל נמען (מנכ"ל/רכז/ראש/כספים של הפרויקט),
// עם הסיכום החכם בגוף כשקיים. לא לקשר ל-'/base-meetings' — loadManualNotifications מוחק אותו!
export async function createBaseMeetingSubmittedNotifications({ meeting, activistName, aiSummary = null }) {
  if (!meeting) return [];
  const projectId = meeting.project_id ?? 1;
  const recipients = await fetchNotificationRecipients(projectId);
  const baseBody = `${activistName || meeting.activist_name || 'פעיל'} מילא דיווח עבור בית מפגש ${meeting.meeting_place_number}, מפגש ${meeting.meeting_number}.`;
  const body = aiSummary ? `${baseBody}\n\nסיכום:\n${aiSummary}` : baseBody;
  return recipients.map(m => createDemoNotification({
    id: notificationId(['base-meeting-submitted-manager', meeting.id, m.id, Date.now()]),
    ...
    priority: 'normal',
    created_at: new Date().toISOString(),
    link: meeting.house_id ? `/meeting-houses/${meeting.house_id}` : null,
  }, { localEcho: false }));
}

// סיכום AI של דיווח קשר — לרכזי הפרויקט של הלקוח בלבד (החלטת מוצר: הפעיל לא רואה סיכומים).
export async function createInteractionSummaryNotifications({ activistName, contact, summary }) {
  if (!contact || !summary) return [];
  const projectId = contact.project_id ?? 1;
  const recipients = await fetchNotificationRecipients(projectId);
  return recipients.filter(r => r.role === 'coord').map(coord => createDemoNotification({
    id: notificationId(['interaction-summary', contact.id, coord.id, Date.now()]),
    type: 'interaction_summary',
    title: `סיכום חדש: דיווח של ${activistName || 'פעיל'} על ${contact.name}`,
    body: summary,
    user_id: coord.id,
    project_id: projectId,
    priority: 'normal',
