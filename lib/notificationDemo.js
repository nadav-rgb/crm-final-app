...
export function createPaymentInteractionNotifications({ interaction, contact, activist, paymentResult }) {
  ...
  // שלושה מצבים, לא שניים: payable ו"מזכה בסכום" הפסיקו להיות נרדפים מאז שטלפוני-ידידותי
  // הפך ל-0 ₪ (Task 1, 2026-08-31) — payable:true,amount:0 הוא מצב תקין (הקשר נספר,
  // פשוט בתעריף אפס), לא כשל. בלי ההפרדה הזו amountText היה נופל ל-'לא מזכה בתשלום'
  // וה-body יוצא "...נכנס לדוח התשלומים בסך לא מזכה בתשלום" — סתירה פנימית
  // (ביקורת קוד, 2026-09-01). reason ריק במצב הזה (calcInteractionPayment מחזיר
  // reason:'' על payable:true) — אין "סיבת אי-זכאות" להציג, כי זו לא דחייה.
  const isPaid     = paymentResult?.payable && paymentResult?.amount > 0;
  const isZeroPaid = paymentResult?.payable && paymentResult?.amount === 0;

  const created = [];

  created.push(createDemoNotification({
    id: notificationId(['paid-interaction-activist', interaction.id, activist.id]),
    type: isPaid ? 'paid_interaction' : 'interaction_saved',
    title: isPaid ? 'הדיווח נכנס לדוח התשלומים' : isZeroPaid ? 'הדיווח נשמר, ללא תשלום' : 'הדיווח נשמר',
    body: isPaid
      ? `הקשר עם ${contact.name} נשמר ונכנס לדוח התשלומים בסך ${paymentResult.amount} ₪.`
      : isZeroPaid
        ? `הקשר עם ${contact.name} נרשם, ללא תשלום (0 ₪).`
