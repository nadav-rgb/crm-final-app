...
async function buildInteractionReportPdf(report, { fontBinary } = {}) {
  ...
  doc.setFontSize(8);
  drawRtlText(doc, 'סוגים נוספים כוללים אירוח שבת וסוגים שאינם פרונטלי, וידאו או טלפוני.', qualityPanelX + panelWidth - 7, panelY + panelHeight - 6);
  ...
  doc.setTextColor(101, 94, 110);
  doc.setFontSize(8);
  drawRtlText(doc, `סה״כ ${segments.trackedClients} לקוחות מתועדים ב־CRM; ${segments.activeClients} מהם יצרו קשר עם פעיל.`, distributionPanelX + panelWidth - 7, panelY + panelHeight - 6);
  ...
  doc.setFontSize(8.5);
  const segmentNote = `ה־850 הוא נתון הנהלה משוער וכולל את קבוצות הקשר האישי. קשר תורני אחד לפחות מסווג לקוח כ״אישי תורני״; ${segments.activeWithoutPersonalQuality} לקוחות פעילים טרם סווגו כאישי תורני או ידידותי.`;
