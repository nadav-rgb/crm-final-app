...
function formatPdfTableRows(rows) {
  return rows.map(row => row.map(formatPdfTableValue));
}
...
async function buildInteractionReportPdf(report, { fontBinary } = {}) {
  ...
  autoTable(doc, {
    ...
    body: formatPdfTableRows([...model.main.rows, model.main.totalRow]),
    theme: 'grid',
  ...
  if (model.mitzvot.rows.length > 0) {
    autoTable(doc, {
      ...
      body: formatPdfTableRows(model.mitzvot.rows),
      theme: 'grid',
