...
const { calcInteractionPayment, comparePaymentOrder, isDerivedInteraction, buildContactContext } = require('./paymentCalc');
...
function payableInteractionsThisMonth(activistId, interactions, contacts, projectId, cfg = undefined) {
  ...
  for (const interaction of myMonthly) {
    ...
    // contactContext מ-כל ה-interactions (לא רק myMonthly) — חלון-3-חודשים ומעבר-לתורני
    // חייבים לראות מעבר לגבול החודש הנוכחי, אותו דפוס כמו calcMonthlyPayment.
    const contactContext = buildContactContext(contact, interaction.contact_id, activistId, interactions);
