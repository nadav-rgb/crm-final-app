...
function buildExecutiveAnalytics(contacts, interactions) {
  ...
  contacts.forEach(contact => {
    if (!contactStates.has(contact.id)) {
      contactStates.set(contact.id, { interactionCount: 0, hasTorani: false, hasFriendly: false });
    }
  });
  ...
  const activeClients = states.filter(state => state.interactionCount > 0).length;
  const distribution = [
    { key: '0', label: '0 קשרים', matches: count => count === 0 },
