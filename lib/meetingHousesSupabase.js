...
// שולח התראת Push מיידית לפעיל (בעת שיבוץ). fire-and-forget — לא חוסם את ה-UI.
// אם לפעיל אין מנוי push (לא נכנס/לא אישר) — השרת מחזיר sent:0 בלי שגיאה.
export async function sendAssignmentPushApi({ activistId, title, body, url }) {
  try {
    const res = await fetch('/api/push/send', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...(await authHeader()) },
      body: JSON.stringify({ activistId: String(activistId), title, body, url }),
    });
    if (!res.ok) { console.error('sendAssignmentPushApi failed', res.status); return null; }
    return await res.json();
  } catch (err) {
    console.error('sendAssignmentPushApi error', err);
    return null;
  }
}
