...
async function loadScopedInteractionReport({ supabase, startDate = '', endDate = '' }) {
  const validation = validateDateRange(startDate, endDate);
  if (!validation.ok) throw new Error(validation.error);
  ...
  if (startDate) interactionsQuery = interactionsQuery.gte('date', startDate);
  if (endDate) interactionsQuery = interactionsQuery.lte('date', endDate);
  const profilesQuery = supabase.from('profiles').select('id,activist_code,name,global_role');
  const membershipsQuery = supabase.from('project_memberships')
  ...
  const [projectResult, contactsResult, interactionsResult, profilesResult, membershipsResult] = await Promise.all([
    projectQuery, contactsQuery, interactionsQuery, profilesQuery, membershipsQuery,
  ]);
  ...
  const activists = (profilesResult.data || [])
