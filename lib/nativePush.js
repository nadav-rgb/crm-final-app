...
export async function initNativePush(activistId) {
  ...
  try {
    ...
    // לחיצה על ההתראה → ניווט ליעד ש-lib/notifyRecipients שלח ב-data.url.
    ...
    if (!navListenerAttached) {
      ...
      await PushNotifications.addListener('pushNotificationActionPerformed', ({ notification }) => {
        ...
        // נתיב פנימי בלבד. ה-payload מגיע מבחוץ, ושורה אחת כאן מונעת ניווט אל מחוץ
        // לאפליקציה אם הוא אי-פעם יגיע פגום או מזויף.
