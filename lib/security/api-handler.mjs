...
import { appendServerAudit } from './auth-service.mjs';
import { requestCorrelationId } from './correlation-id.mjs';

const SAFE_METHODS = new Set(['GET', 'HEAD', 'OPTIONS']);
const AUDITED_DENIALS = new Set(['CAPABILITY_DENIED', 'MFA_REQUIRED', 'CSRF_DENIED', 'RATE_LIMITED']);

export function secureHandler(options, businessHandler, dependencies = {}) {
  const appendAuditFn = dependencies.appendAudit ?? appendServerAudit;
  if (typeof businessHandler !== 'function' || typeof appendAuditFn !== 'function') {
  ...
  return async function securedRoute(req, res) {
    ...
    try {
      ...
      let error = caught;
      if (AUDITED_DENIALS.has(error?.code)) {
        try {
          await appendAuditFn({
