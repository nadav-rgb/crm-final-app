...
function canonicalNetworkBucket(value) {
  if (value === 'ip:unknown') return value;
  if (/^ip4:(?:25[0-5]|2[0-4]\d|1?\d?\d)\.(?:25[0-5]|2[0-4]\d|1?\d?\d)\.(?:25[0-5]|2[0-4]\d|1?\d?\d)\.0\/24$/.test(value ?? '')) {
    return value;
  }
  if (/^ip6:(?:[0-9a-f]{4}:){3}[0-9a-f]{4}::\/56$/.test(value ?? '')) return value;
  return 'ip:unknown';
}

function opaqueRateBucket(value) {
  const normalized = typeof value === 'string' && value.length > 0
    ? value.normalize('NFKC').slice(0, 512)
    : 'invalid';
  return `sha256:${createHash('sha256').update(normalized).digest('base64url').slice(0, 24)}`;
}

export function buildRateLimitKey({ networkKey, identity, session } = {}) {
  const parts = [
    `network=${canonicalNetworkBucket(networkKey)}`,
    `identity=${opaqueRateBucket(identity)}`,
  ];
  if (session != null) parts.push(`session=${opaqueRateBucket(session)}`);
  return parts.join('|');
}
...
function isMfaProtected(profile) {
  return profile?.globalRole === 'ceo' || profile?.role === 'ceo'
    || (profile?.projects ?? []).some((project) => project?.role === 'head');
}

function sessionRole(profile) {
  if (profile?.globalRole === 'ceo' || profile?.role === 'ceo') return 'ceo';
  if ((profile?.projects ?? []).some((project) => project?.role === 'head')) return 'head';
  return canonicalRole(profile);
}
...
function mfaStatusSession(session, mfaEnrolled) {
  return {
    ok: true,
    cookieValue: session.id,
    csrfToken: session.csrfToken,
    ...
    aal: 1,
    mfaEnrolled: Boolean(mfaEnrolled),
  };
}

async function hasVerifiedMfaFactor(provider, session) {
  if (typeof provider.listMfaFactors !== 'function') return false;
  const factors = await provider.listMfaFactors(session);
...
export function createAuthService({ identityStore, provider, sessions, rateLimiter, audit, clock = { now: () => new Date() } }) {
  ...
  async function login({ username, password, ipKey = 'unknown' }) {
    ...
    await consume('login', buildRateLimitKey({
      networkKey: ipKey, identity: normalizedUsername || 'invalid',
    }), 5, 15 * 60);
    ...
    const role = sessionRole(profile);
    const aal = providerSession.aal === 2 ? 2 : 1;
    const authState = isMfaProtected(profile) && aal < 2 ? 'mfa_required' : 'active';
    const session = await sessions.create({
      ...
      aal,
      authState,
      mfaProtected: isMfaProtected(profile),
      factorFingerprint: providerSession.factorFingerprint ?? null,
    ...
    if (authState === 'mfa_required') {
      return mfaStatusSession(session, await hasVerifiedMfaFactor(provider, session));
    }
    return publicSession(session, profile);
  }
  ...
  async function resume(cookieValue) {
    ...
    const protectedSession = isMfaProtected(profile);
    const authState = protectedSession && session.aal < 2 ? 'mfa_required' : 'active';
    const rotated = await sessions.rotate(session, {
      role: sessionRole(profile), authState, mfaProtected: protectedSession,
    }, 'session_resume');
    if (authState === 'mfa_required') {
      return mfaStatusSession(rotated, await hasVerifiedMfaFactor(provider, rotated));
    }
    return publicSession(rotated, profile);
  }
  ...
  async function challengeMfa(session, factorId, { ipKey = 'unknown' } = {}) {
    if (!['mfa_required', 'active'].includes(session?.authState)) throw authInvalid();
    await consume('mfa_challenge', buildRateLimitKey({
      networkKey: ipKey, identity: session.userId, session: session.idHash,
    }), 5, 10 * 60);
    let selectedFactorId = factorId;
    if (!selectedFactorId) {
      const factors = typeof provider.listMfaFactors === 'function'
        ? await provider.listMfaFactors(session)
        : [];
      selectedFactorId = factors?.[0]?.id;
    }
    if (!selectedFactorId) throw authInvalid();
    const challenge = await provider.challengeMfa(session, selectedFactorId);
    return { ...challenge, factorId: selectedFactorId };
  }

  async function verifyMfa(session, input, { ipKey = 'unknown' } = {}) {
    ...
    await consume('mfa_verify', buildRateLimitKey({
      networkKey: ipKey, identity: session.userId, session: session.idHash,
    }), 5, 10 * 60);
    let verified;
    try {
    ...
    const rotated = await sessions.rotate(session, {
      ...
      factorFingerprint: verified.factorFingerprint ?? session.factorFingerprint,
    }, 'mfa_verified');
    await auditSafe(audit, {
      actorUserId: session.userId, action: 'auth.mfa.verify', resourceType: 'auth',
  ...
  async function requestPasswordReset({ username, ipKey = 'unknown' }) {
    const normalizedUsername = normalizeUsername(username);
    await consume('password_reset', buildRateLimitKey({
      networkKey: ipKey, identity: normalizedUsername || 'invalid',
    }), 3, 60 * 60);
    const identity = normalizedUsername ? await identityStore.resolve(normalizedUsername) : null;
    if (identity) {
...
function factorFingerprint(factors) {
  const verifiedIds = (factors ?? [])
    .filter((factor) => factor?.status === 'verified' && typeof factor?.id === 'string')
    .map((factor) => factor.id)
    .sort();
  return createHash('sha256').update(JSON.stringify(verifiedIds)).digest('base64url');
}

async function providerMfaState(client) {
  const [assurance, listed] = await Promise.all([
    client.auth.mfa.getAuthenticatorAssuranceLevel(),
    client.auth.mfa.listFactors(),
  ]);
  if (assurance.error || listed.error) throw assurance.error ?? listed.error;
  const factors = listed.data?.totp ?? [];
  return {
    aal: assurance.data?.currentLevel === 'aal2' ? 2 : 1,
    nextAal: assurance.data?.nextLevel === 'aal2' ? 2 : 1,
    factorFingerprint: factorFingerprint(factors),
    factors: factors
...
function createSupabaseProvider(env) {
  ...
  return Object.freeze({
    async signInWithPassword(credentials) {
      ...
      const mfaState = await providerMfaState(client);
      return {
        userId: data.user.id,
    ...
    async listMfaFactors(session) {
      const client = await authenticatedClient(session);
      return (await providerMfaState(client)).factors;
    },
    ...
    async verifyMfa(session, input) {
      ...
      const mfaState = await providerMfaState(client);
      if (mfaState.aal !== 2 || mfaState.nextAal !== 2) throw new Error('AAL2 assurance missing');
      return {
        ...mfaState,
...
export function getDefaultAuthRuntime() {
  ...
  defaultRuntime = Object.freeze({ service, env, sessions, rateLimiter, audit });
  return defaultRuntime;
}

export async function consumeServerRateLimit(input) {
  return getDefaultAuthRuntime().rateLimiter.consume(input);
}

export async function appendServerAudit(event) {
  return getDefaultAuthRuntime().audit.append(event);
}
