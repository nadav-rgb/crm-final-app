...
export const tourCommandSchema = z.object({
  ...
  settlement: z.string().trim().max(120).optional(),
  date: isoDateSchema.optional(),
...
export const tourUpdateCommandSchema = tourCommandSchema.omit({ id: true }).partial().strict().refine(
  (value) => Object.keys(value).length > 0,
  { message: 'At least one editable field is required' },
);
...
export const tourIdCommandSchema = z.object({ tourId: boundedId }).strict();
...
export const notificationEventSchema = z.object({
  eventType: z.enum([
    ...
    'tour_reported', 'interaction_created', 'base_meeting_reported', 'mitzvot_updated', 'self_test',
  ]),
