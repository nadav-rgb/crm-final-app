...
import { projectMembersAreActive } from './route-support.mjs';
...
export async function assignHouse(context, houseId, input) {
  ...
  const areActiveMembers = (projectId, ids) => projectMembersAreActive(context, projectId, ids);
  const command = await assignHouseCommand(context, house, input, { areActiveMembers });
  const { data: assigned, error } = await context.db.rpc('app_assign_meeting_house', {
    p_house_id: String(house.id),
    p_assigned_user_ids: command.assigned_user_ids,
  });
  dbError(error);
  if (assigned !== true) throw new SecurityError(404, 'NOT_FOUND', 'Meeting house was not found');
  return getHouse(context, house.id);
}
...
export async function upsertHouse(context, input) {
  ...
  if (command.id !== undefined) {
    const { data: existing, error: existingError } = await context.db.from('meeting_houses')
      .select('id,project_id').eq('id', command.id).maybeSingle();
    dbError(existingError);
    if (existing) {
      if (Number(existing.project_id) !== Number(command.project_id)) {
        throw new SecurityError(404, 'NOT_FOUND', 'Meeting house was not found');
      }
      const assignedUserIds = command.assigned_user_ids;
      const editable = { ...command };
      delete editable.id;
      delete editable.project_id;
      delete editable.assigned_user_ids;
      const { data: assigned, error: assignError } = await context.db.rpc('app_assign_meeting_house', {
        p_house_id: String(existing.id),
        p_assigned_user_ids: assignedUserIds,
      });
      dbError(assignError);
      if (assigned !== true) throw new SecurityError(404, 'NOT_FOUND', 'Meeting house was not found');
      const { data, error } = await context.db.from('meeting_houses').update(editable).eq('id', existing.id)
  ...
  const { data, error } = await context.db.from('meeting_houses').insert(command)
