...
export function buildBonusCandidates({ activistCode, contacts = [], interactions = [], config, cancelledKeys = new Set(), period }) {
  ...
  for (const contact of contacts) {
    // Contact-owned bonuses follow the contact's current activist assignment.
    // Interaction-owned bonuses below remain attributed to the historical actor.
    if (Number(contact.activist_id) !== Number(activistCode)) continue;
...
export function bonusCancellationCommand(context, input, dependencies = {}) {
  ...
  const { contact, candidate, beneficiaryUserId } = dependencies;
  if (!contact || String(contact.id) !== key.contactId
    || candidate?.key !== parsed.data.bonusKey || !UUID.test(String(beneficiaryUserId ?? ''))) {
...
export async function listOwnBonusCancellations(context, input) {
  ...
  const data = await readAllPages(() => context.db.from('bonus_cancellations').select('bonus_key')
    .eq('beneficiary_user_id', command.user_id)
    .like('bonus_key', `${command.activist_code}|%|${command.month_key}`), 'bonus_key');
  const keys = [];
...
async function scopedDirectoryProfileByActivistCode(context, projectId, activistCode) {
  ...
  dbError(error);
  const row = (data ?? []).find((entry) => Number(entry.activist_code) === Number(activistCode));
  return row ? { id: row.user_id, activist_code: row.activist_code } : null;
}

async function readAllPages(queryFactory, orderColumn = 'id') {
  const rows = [];
...
async function readRowsForValues(values, queryFactory, orderColumn) {
  const rows = [];
  for (let offset = 0; offset < values.length; offset += POSTGREST_IN_CHUNK_SIZE) {
    const chunk = values.slice(offset, offset + POSTGREST_IN_CHUNK_SIZE);
    rows.push(...await readAllPages(() => queryFactory(chunk), orderColumn));
  }
  return rows;
}
...
async function bonusSourceRows(context, command) {
  ...
  const interactions = await readAllPages(() => {
    let query = context.db.from('interactions').select(BONUS_INTERACTION_COLUMNS)
      .eq('actor_user_id', command.user_id);
    if (command.project_id != null) query = query.eq('project_id', command.project_id);
    return query;
  });
  const assignedContacts = await readAllPages(() => {
    let query = context.db.from('contacts').select(BONUS_CONTACT_COLUMNS)
      .eq('assigned_user_id', command.user_id);
    if (command.project_id != null) query = query.eq('project_id', command.project_id);
    return query;
  });
  const assignedIds = new Set(assignedContacts.map((contact) => String(contact.id)));
  const historicalContactIds = [...new Set(interactions.map((interaction) => String(interaction.contact_id)))]
    .filter((id) => id && !assignedIds.has(id));
  const historicalContacts = historicalContactIds.length === 0 ? [] : await readRowsForValues(
    historicalContactIds,
    (ids) => {
      let query = context.db.from('contacts').select(BONUS_CONTACT_COLUMNS).in('id', ids);
      if (command.project_id != null) query = query.eq('project_id', command.project_id);
      return query;
    },
    'id',
  );
  const contacts = [...assignedContacts, ...historicalContacts];
  ...
  return { target, contacts, interactions, config };
}
...
export async function listBonusCandidates(context, input) {
  ...
  const cancellations = await readRowsForValues(
    preliminary.map((candidate) => candidate.key),
    (keys) => context.db.from('bonus_cancellations').select('bonus_key').in('bonus_key', keys),
    'bonus_key',
  );
...
export async function createBonusCancellation(context, input) {
  ...
  if (!contact) throw new SecurityError(404, 'NOT_FOUND', 'Bonus was not found');
  assertBonusCancellationAccess(context, contact);
  const beneficiary = await scopedDirectoryProfileByActivistCode(
    context, Number(contact.project_id), parsedKey.activistCode,
  );
  if (!beneficiary) throw new SecurityError(404, 'NOT_FOUND', 'Bonus was not found');
