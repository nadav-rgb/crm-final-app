...
const BONUS_TYPES = new Set(['בונוס-לימוד-4', 'בונוס-לימוד-6', 'בונוס-מצוות', 'בונוס-חדש']);
...
export function assertExpenseAccess(context, row, action = 'read') {
  ...
  if (context.globalRole === 'ceo') {
    if (context.aal < 2) throw new SecurityError(403, 'MFA_REQUIRED', 'Multi-factor authentication is required');
    if (action !== 'read' && row.actor_user_id !== context.userId) throw new SecurityError(403, 'CAPABILITY_DENIED', 'Access is denied');
    return;
  }
  ...
  if (action === 'read') {
    if (['activist', 'coord'].includes(active.role) || authorize(context, 'finance:read', { projectId: row.project_id })) return;
  ...
  for (const contact of contacts) {
    ...
    for (const event of jsonValue(contact.mitzvot_history, [])) {
      ...
      if (event?.mitzva && Number(event.to ?? 0) > Number(event.from ?? 0) && inPeriod(eventDate, start, end)) {
        add('בונוס-מצוות', contactId, config.bonus_mitzvot_level);
      }
    }
...
export async function deleteExpense(context, id) {
  ...
  const { data, error } = await context.db.from('expenses').delete().eq('id', id).eq('actor_user_id', context.userId).select('id').maybeSingle();
  dbError(error);
  if (!data) throw new SecurityError(404, 'NOT_FOUND', 'Expense was not found');
  return { deleted: true };
}
...
async function bonusSourceRows(context, command) {
  const { data: target, error: targetError } = await context.db.from('profiles')
    .select('id,activist_code').eq('id', command.user_id).maybeSingle();
  dbError(targetError);
  ...
  const { start, end } = periodBounds(command.period);
  let interactionQuery = context.db.from('interactions').select(BONUS_INTERACTION_COLUMNS)
    .eq('actor_user_id', command.user_id).gte('date', start).lt('date', end);
...
export async function createBonusCancellation(context, input) {
  ...
  const { data: actor, error: actorError } = await context.db.from('profiles')
    .select('id,activist_code').eq('id', context.userId).maybeSingle();
  dbError(actorError);
  if (!actor) throw new SecurityError(409, 'IDENTITY_MAPPING_REQUIRED', 'Actor identity mapping is unavailable');
  const command = bonusCancellationCommand(context, input, {
    contact, candidate, actorActivistCode: actor.activist_code,
  });
  const { data, error } = await context.db.from('bonus_cancellations').insert(command)
    .select('id,bonus_key').maybeSingle();
  if (error?.code === '23505') throw new SecurityError(409, 'BONUS_ALREADY_CANCELLED', 'Bonus is already cancelled');
  dbError(error);
  if (!data) throw new SecurityError(404, 'NOT_FOUND', 'Bonus was not found');
  return { cancelled: true, bonus: { key: data.bonus_key, type: candidate.type, amount: candidate.amount } };
}
