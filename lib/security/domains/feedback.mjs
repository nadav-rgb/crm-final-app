...
export async function reviewFeedback(context, input) {
  ...
  const { data: reviewed, error } = await context.db.rpc('app_review_feedback', {
    p_feedback_id: input.id,
    p_status: input.status,
  });
  dbError(error);
  if (reviewed !== true) throw new SecurityError(404, 'NOT_FOUND', 'Feedback was not found');
  const { data, error: reloadError } = await context.db.from('feedback_reports')
    .select(COLUMNS).eq('id', input.id).maybeSingle();
  dbError(reloadError);
