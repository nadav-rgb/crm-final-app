...
import { contactCreateSchema, contactUpdateSchema, paginationSchema } from '../schemas.mjs';
...
export async function listContacts(context) {
  return (await listContactsPage(context, { limit: 100 })).items;
}
...
export async function listContactsPage(context, input = {}) {
  requireContext(context);
  ...
  let query = context.db.from('contacts').select(LIST_COLUMNS).eq('is_active', true);
  if (context.globalRole !== 'ceo') {
  ...
  for (const row of items) assertContactAccess(context, 'read', row);
  return {
    items: items.map(toContactListDto),
