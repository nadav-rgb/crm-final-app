...
function getMonthlyTotalForActivist(activistId, allInteractions, allContacts, cfg = DEFAULTS, period = null) {
  ...
  for (const interaction of myMonthly) {
    ...
    const contactContext = { joinedAt: contact?.joined_at ?? null, allInteractionsWithContact: (allInteractions || []).filter(i => i.contact_id === interaction.contact_id && i.activist_id === activistId) };
...
module.exports = { calcMonthlyPayment, calcInteractionPayment, getMonthlyTotalForActivist, paidBefore, calcConsultantDashboard, makeBonusKey, isDerivedInteraction, comparePaymentOrder, interactionBasePrice, deriveMitzvotBonuses, previewNewMitzvotBonusCount, previewNewMitzvotBonusChanges, resolvePeriod, DEFAULTS, MITZVOT_BONUS_PER_LEVEL, NEW_PARTICIPANT_BONUS, BASE_PRICES, MONTHLY_PAYMENT_CAP, PAID_PROJECT_IDS, FRIENDLY_ELIGIBLE_MONTHS, FRIENDLY_FRONTAL_MONTHLY_CAP, TORANI_BONUS_AMOUNT, TORANI_BONUS_MONTHS, isoYearMonth, monthsBetween };
