 ...
 * כולל בונוסים
 */
function calcMonthlyPayment(activistId, interactions, contacts, mitzvotBonuses = [], newParticipantBonuses = [], cfg = DEFAULTS, cancelledBonusKeys = EMPTY_SET) {
  ...
  const monthlyInteractions = interactions.filter(i => {
    ...
    return i.activist_id === activistId &&
           d.getMonth() === month &&
           d.getFullYear() === year &&
           isPaidInteraction(i); // אחדות יהודית + נעים להכיר, תקרות משותפות
  });
 ...
 */
function calcConsultantDashboard(activistId, interactions, contacts, mitzvotBonuses = [], newParticipantBonuses = [], cfg = DEFAULTS, cancelledBonusKeys = EMPTY_SET) {
  const now = new Date();
  const month = now.getMonth();
  const year  = now.getFullYear();
  ...
  const salary = calcMonthlyPayment(activistId, interactions, contacts, mitzvotBonuses, newParticipantBonuses, cfg, cancelledBonusKeys);
...
module.exports = { calcMonthlyPayment, calcInteractionPayment, getMonthlyTotalForActivist, calcConsultantDashboard, makeBonusKey, DEFAULTS, MITZVOT_BONUS_PER_LEVEL, NEW_PARTICIPANT_BONUS, BASE_PRICES, MONTHLY_PAYMENT_CAP, PAID_PROJECT_IDS };
