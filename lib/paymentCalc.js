...
function calcInteractionPayment(interaction, prevContactMonthly, isHighPotential, prevActivistMonthly = null, cfg = DEFAULTS) {
  ...
  const frontalCountAll = activistMonthly.filter(i => i.type === 'פרונטלי' && (i.duration_minutes ?? 0) >= MIN_DUR).length;
  ...
  const contactFrontal = prevContactMonthly.filter(i => i.type === 'פרונטלי' && (i.duration_minutes ?? 0) >= MIN_DUR).length;
 ...
 */
function getMonthlyTotalForActivist(activistId, allInteractions, allContacts, cfg = DEFAULTS) {
  const now   = new Date();
 ...
 * כולל בונוסים
 */
function calcMonthlyPayment(activistId, interactions, contacts, mitzvotBonuses = [], newParticipantBonuses = [], cfg = DEFAULTS) {
  ...
  const monthlyInteractions = interactions.filter(i => {
    ...
    return i.activist_id === activistId &&
           d.getMonth() === month &&
           d.getFullYear() === year &&
           i.project_id === 1; // אחדות יהודית בלבד (project_id=1 ב-Supabase)
  });
 ...
 */
function calcConsultantDashboard(activistId, interactions, contacts, mitzvotBonuses = [], newParticipantBonuses = [], cfg = DEFAULTS) {
  const now = new Date();
  const month = now.getMonth();
  const year  = now.getFullYear();

  // רק קשרים של היועץ, פרויקט אחדות יהודית (1), בחודש הקלנדרי הנוכחי (איפוס חודשי אוטומטי).
  ...
  const frontalDone     = monthly.filter(i => i.type === 'פרונטלי' && i.quality !== 'רב משתתפים').length;
  const phoneToraniDone = monthly.filter(i => i.type === 'טלפוני'  && i.quality === 'תורני').length;
  const multiDone       = monthly.filter(i => i.type === 'פרונטלי' && i.quality === 'רב משתתפים').length;

  const counters = {
    frontal:     { done: frontalDone,     cap: cfg.MONTHLY_CAPS.frontal, label: 'פגישות פרונטליות' },
    phoneTorani: { done: phoneToraniDone, cap: cfg.MONTHLY_CAPS.phone,   label: 'שיחות טלפון תורניות' },
    multi:       { done: multiDone,       cap: cfg.MONTHLY_CAPS.multi,   label: 'מפגשים רבי-משתתפים' },
  };

  // שכר — מהמנוע האמיתי, כך שתואם בדיוק לעמוד התשלומים של הרכז.
  const salary = calcMonthlyPayment(activistId, interactions, contacts, mitzvotBonuses, newParticipantBonuses, cfg);
...
module.exports = { calcMonthlyPayment, calcInteractionPayment, getMonthlyTotalForActivist, calcConsultantDashboard, DEFAULTS, MITZVOT_BONUS_PER_LEVEL, NEW_PARTICIPANT_BONUS, BASE_PRICES, MONTHLY_PAYMENT_CAP };
