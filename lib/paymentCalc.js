...
function calcMonthlyPayment(activistId, interactions, contacts, mitzvotBonuses = [], newParticipantBonuses = []) {
  ...
  // [DIAG] מעקב שלב-שלב אחרי הסינון
  const step1 = interactions.filter(i => i.activist_id === activistId);
  console.log(`[CALC-DIAG] activist ${activistId} — after activist_id filter: ${step1.length}`, step1.map(i=>({id:i.id,project_id:i.project_id,date:i.date,type:i.type,quality:i.quality,duration_minutes:i.duration_minutes})));
  const step2 = step1.filter(i => i.project_id === 1);
  console.log(`[CALC-DIAG] after project_id===1 filter: ${step2.length}`);
  const step3 = step2.filter(i => { const d=new Date(i.date); return d.getMonth()===month && d.getFullYear()===year; });
  console.log(`[CALC-DIAG] after month/year filter (${month}/${year}): ${step3.length}`);
