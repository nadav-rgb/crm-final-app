...
// ביטול סיור — status → 'cancelled'. הסיור נשאר בהיסטוריה, יוצא מחישוב השכר,
// והקהל מקבל התראה. reason אופציונלי ומופיע בגוף ההתראה.
// מחזיר { tour, notified } או null בכישלון.
export async function cancelTourApi(tourId, reason) {
  const res = await fetch('/api/tours/cancel', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...(await authHeader()) },
    body: JSON.stringify({ tourId: String(tourId), reason: reason || '' }),
  });
  if (!res.ok) { console.error('cancelTourApi failed', res.status, await res.text().catch(() => '')); return null; }
  const { tour: saved, notified } = await res.json();
  return saved ? { tour: mapRowToTour(saved), notified: notified || [] } : null;
}

// מחיקה לצמיתות — רק לסיור שנוצר בטעות. השרת חוסם אם יש דיווח או לקוחות מקושרים.
// מחזיר { ok: true, notified } בהצלחה, או { ok: false, message } עם הסיבה שהמחיקה נחסמה.
export async function deleteTourApi(tourId) {
  const res = await fetch('/api/tours/delete', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...(await authHeader()) },
  ...
  if (!res.ok) {
    console.error('deleteTourApi failed', res.status, payload);
    return { ok: false, message: payload.message || 'מחיקת הסיור נכשלה — נסה שוב' };
  }
  return { ok: true, notified: payload.notified || [] };
}
