...
export default function LandingPage() {
  const { currentUser, activeProject, filterProject } = useAuth();
  ...
  // Project filter is owned by DesktopLayout's sidebar via auth
  const projId = (filterProject === null || filterProject === 0) ? null : filterProject;
  ...
  const filteredInteractions = isActivist
    ...
    : projId === null
      ...
      : interactions.filter(i => {
          const contact = contacts.find(c => c.id === i.contact_id);
          return contact?.project_id === projId;
        });
  ...
  const filteredContacts = isActivist
    ...
    : projId === null
      ? contacts
      : contacts.filter(c => c.project_id === projId);
  ...
  // Drag-to-scroll for recent activity row
  ...
  const projName = isActivist
    ? activeProject?.name
    : projId === null ? 'כל הפרויקטים' : (PROJECTS.find(p => p.id === projId)?.name ?? 'כל הפרויקטים');

  return (
    <DesktopLayout
      title="מרכז הפעילות"
      subtitle={`ברוך הבא, ${currentUser?.name} · ${projName}`}
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginTop: 8, marginBottom: 44 }}>
        {stats.map(({ num, label, color, rgb }) => (
          ...
          >
            ...
            <div style={{ fontSize: 64, fontWeight: 800, color: '#0f172a', lineHeight: 1, letterSpacing: '-0.04em', fontVariantNumeric: 'tabular-nums' }}>
              {num}
            </div>
            <div style={{ fontSize: 12, fontWeight: 500, color: '#94a3b8', letterSpacing: '0.02em', marginTop: 10 }}>
              {label}
            </div>
          </div>
        ))}
            ...
            .map(msg => (
              <div key={msg.id} style={{ borderBottom: '0.5px solid rgba(196,122,46,0.15)', paddingBottom: 12, marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#3d2c1e' }}>{msg.pinned && '📌 '}{msg.title}</div>
