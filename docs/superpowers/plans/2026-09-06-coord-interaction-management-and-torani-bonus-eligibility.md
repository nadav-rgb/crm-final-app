...
- Modify: `scripts/verify-payment-order.cjs:577-654` (עדכון בדיקות קיימות שישברו), הוספת בדיקות חדשות
...
⚠️ **קריטי — הבדיקות הקיימות ל-`deriveToraniBonuses` (שורות 577-654) ישברו** ברגע שמוסיפים את הפילטר, כי `contactsTB` שם הן `[{ id: 1, name: '...' }, { id: 2, name: '...' }]` — **בלי `joined_at`**, וללא `joined_at` הלקוח לא זכאי בכלל תחת הכלל החדש. הפתרון: להזיז את כל התאריכים בבלוק הזה **קדימה ב-6 חודשים בדיוק** (משמר את כל יחסי-הרצף/פערים המקוריים) ולהוסיף `joined_at: '2026-09-01'` לשני הלקוחות. טבלת ההזזה המדויקת (לשימוש ב-Step 1):
...
- [ ] **Step 1: עדכן את הבדיקות הקיימות ב-`scripts/verify-payment-order.cjs` (שורות 577-654) כדי שישרדו את התיקון**

מצא את הבלוק שמתחיל ב-`// בונוס תורני — 3 חודשים רצופים, פעם אחת בלבד ללקוח (2026-08-31).` (שורה 577) ומסתיים ב-`}` (שורה 654). **החלף את כל הבלוק** בגרסה הבאה (זהה בדיוק ללוגיקה המקורית, רק עם תאריכי-חודש מוזזים ב-6 ו-`joined_at` נוסף):
