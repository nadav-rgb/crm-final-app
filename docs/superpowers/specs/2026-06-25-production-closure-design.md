...
## הערות בודקים (קבוצת ווטסאפ, 28.5–1.6.2026) — מסונן ומשולב

נכנסים לסגירה:
- **B1 (קריטי, ליבה עסקית)**: `paymentCalc.js` — תשלום על מפגשים חוזרים עם אותו לקוח לא נרשם (מפגש 1 משלם, 2-3 "ללא תשלום"), "מפגש שבועי" ובונוס התמדה אחרי 3 מפגשים לא מחושבים. דווח ע"י רפאל + מוטי ג' (פעמיים), תוקן חלקית בעבר ועדיין שבור. → שלב 2, עדיפות ראשונה. כולל הבהרת תצוגה "קשרים מזכים בתשלום" (U3).
- **B2**: אין UI לסמן משימה/תזכורת כ"בוצע" — מופיעה כ"איחור" ללא דרך להשלים. → שלב 2.
- **B3**: הקלדה בדיבור (מיקרופון) נכשלת עם שגיאת הרשאה — קשור Techloq/overlay. מעטפת Capacitor (שלב 3) נותנת הרשאת מיקרופון נייטיב שעשויה לפתור. → לאמת על המכשיר אחרי בניית ה-APK.
- **F1**: עריכת פרטי לקוח קיים + מחיקת לקוח (soft-delete עם הרשאה). אין UI כזה היום. → שלב 2.
- **U1**: קוביות הסטטיסטיקה במרכז הפעילות + אזור אישי → לחיצות מנווטות לעמוד המתאים. → שלב 4.
- **U2**: גילוי "עדכון התקדמות רוחנית (מצוות)" — הפיצ'ר קיים אך לא נגיש; לשפר נראות/כניסה. → שלב 4.

נדחה / לא רלוונטי:
- **F2 (נדחה)**: בקשה להפוך סימון מצוות לאופציונלי ברישום — **נשאר חובה** (החלטת מוצר, מבטיח נתונים מלאים).
- **"פתח יעד"** — כבר מוסתר בקוד (`notifications.jsx:49`); הבודק ראה גרסה ישנה.
- **צ'אט לא שומר** — מכוון, מוסתר "בקרוב".
- **"לינק לא נפתח" / "פעילות של אחרים" / "הכל ריק"** — Techloq + reset יזום, לא באגים באפליקציה.

ניקיון צדדי (מחוץ לסקופ אך מומלץ): 4 תיקיות `.claude/worktrees/` ישנות — שאריות git.
