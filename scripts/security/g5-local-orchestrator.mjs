...
import { assertSafeTestTarget, inspectLocalStackIdentity } from './verify-rls-live.mjs';
...
export function registerSyntheticRunResources({ registry, rowsByTable, actors }) {
  ...
  for (const actor of actors.values()) {
    ...
    registry.register({
      kind: 'auth-identity', schema: 'app_private', table: 'auth_identities', column: 'user_id', value: actor.id,
    });
  }
  ...
  return Object.freeze({
    async start() {
      ...
      try {
        ...
        command(['stop', '--workdir', resolvedProject, '--no-backup'], 'failed-start cleanup');
        active = false;
        throw cause;
      }
    },
    async stop() {
      ...
      command(['stop', '--workdir', resolvedProject, '--no-backup'], 'stop');
      active = false;
      verifiedTarget = null;
    },
  });
...
async function applyAndVerify(database, step) {
  ...
  await database.inventory(`after-${step.id}`);
}
...
export async function runG5LocalLifecycle({
  ...
  try {
    ...
    await applyAndVerify(database, migrationPlan[0]);
    await database.resetToLegacy();
    await seedLegacy({ database, target, actors });
    ...
    for (const step of migrationPlan) await applyAndVerify(database, step);
    ...
    return Object.freeze({ completed: true, evidence, cleanup: cleanupResult });
  } catch (cause) {
...
export async function runConfiguredLocalG5({
  ...
  try {
    ...
    await emitEvidence(config.evidencePath, evidence);
    return Object.freeze({
